...
import type {BranchSuggestInput, BranchSuggestion, WorkflowStrategy} from './types';
import {
	isGitRepository,
	getCurrentBranch,
	getDefaultBranch,
...
async function analyzeWorkflowStrategy(): Promise<{
	...
	try {
		...
		return {
			strategy: 'feature-branch',
			rationale: 'Default recommendation: Feature Branch workflow suits most projects',
		};
	} catch {
		return {
			strategy: 'feature-branch',
			rationale: 'Unable to analyze repository; recommending Feature Branch workflow',
		};
	}
}
...
/**
 * Handler for the git_branch_suggest tool
 */
const executeGitBranchSuggest = async (args: BranchSuggestInput): Promise<string> => {
	...
	responseLines.push(`  ${suggestedName}${branchExists ? ' (already exists!)' : ''}`);
	responseLines.push('');
	...
	inputSchema: jsonSchema<BranchSuggestInput>({
		...
		properties: {
			...
			ticketId: {
				type: 'string',
				description:
					'Optional ticket/issue ID (e.g., "PROJ-123", "GH-456")',
			},
		},
...
const GitBranchSuggestFormatter = React.memo(
	({args, result}: {args: BranchSuggestInput; result?: string}) => {
		...
		if (result) {
			const branchMatch = result.match(/--- Suggested Branch Name ---\n\s+(\S+)/);
...
export const gitBranchSuggestTool: ToolDefinition = {
	name: 'git_branch_suggest',
	tool: gitBranchSuggestCoreTool,
	handler: executeGitBranchSuggest,
	formatter,
	validator,
