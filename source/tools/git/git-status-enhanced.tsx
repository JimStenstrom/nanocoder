 ...
 * Get stash list
 */
async function getStashList(): Promise<Array<{index: number; message: string}>> {
	try {
 ...
 */
async function getRecentCommits(count: number = 5): Promise<Array<{hash: string; subject: string; date: string}>> {
	try {
...
function generateSuggestions(
	...
	// Sync status
	if (status.behind > 0) {
		suggestions.push(`Your branch is ${status.behind} commit(s) behind upstream`);
...
/**
 * Handler for the git_status_enhanced tool
 */
const executeGitStatusEnhanced = async (args: EnhancedStatusInput): Promise<string> => {
...
export const gitStatusEnhancedTool: ToolDefinition = {
	name: 'git_status_enhanced',
	tool: gitStatusEnhancedCoreTool,
	handler: executeGitStatusEnhanced,
	formatter,
	validator,
