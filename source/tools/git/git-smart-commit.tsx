...
import type {SmartCommitInput, GeneratedCommit, CommitType} from './types';
import {
	isGitRepository,
...
function generateCommitMessage(
	...
	const {suggestedType, suggestedScope, files, isBreakingChange, breakingChangeReason} = analysis;
 ...
 */
function generateSubject(type: CommitType, files: Array<{path: string; status: string}>): string {
 ...
 */
function generateBody(files: Array<{path: string; status: string; additions: number; deletions: number}>): string {
	const lines: string[] = [];
...
/**
 * Handler for the git_smart_commit tool
 */
const executeGitSmartCommit = async (args: SmartCommitInput): Promise<string> => {
	...
	if (args.dryRun) {
		...
		responseLines.push(`git commit -m "${commit.fullMessage.replace(/"/g, '\\"').replace(/\n/g, '\\n')}"`);
	} else {
		...
		try {
			...
			responseLines.push(`Error creating commit: ${error instanceof Error ? error.message : 'Unknown error'}`);
		}
	}
...
const GitSmartCommitFormatter = React.memo(
	({args, result}: {args: SmartCommitInput; result?: string}) => {
		...
		if (result) {
			...
			const typeMatch = result.match(/^(feat|fix|docs|style|refactor|perf|test|build|ci|chore|revert)/m);
			if (typeMatch) commitType = typeMatch[1];

			isBreaking = result.includes('BREAKING CHANGE') || result.includes('breaking change');
		}
...
export const gitSmartCommitTool: ToolDefinition = {
	name: 'git_smart_commit',
	tool: gitSmartCommitCoreTool,
	handler: executeGitSmartCommit,
	formatter,
	validator,
};
