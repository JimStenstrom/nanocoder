...
function generatePRTitle(
	...
	for (const category of titlePriority) {
		...
		if (items && items.length > 0) {
			...
			const prefix = category === 'feat' ? 'Add' : category === 'fix' ? 'Fix' : 'Update';
...
/**
 * Handler for the git_create_pr tool
 */
const executeGitCreatePR = async (args: CreatePRInput): Promise<string> => {
	...
	responseLines.push('');
	responseLines.push('Or copy the template above and create the PR manually on GitHub.');
...
export const gitCreatePRTool: ToolDefinition = {
	name: 'git_create_pr',
	tool: gitCreatePRCoreTool,
	handler: executeGitCreatePR,
	formatter,
	validator,
};
